<template>
  <div class="editor">
    ...
    <el-table ref="dragTable" class="block-center" :data="option.setting" row-key="id" border fit size="mini"
      ...
      <el-table-column align="center" label="输入框提示">
        <template slot-scope="{row,$index}">
          <div style="display: flex;flex-flow: row nowraps;align-items: center;">
            <el-input v-model="row.value"></el-input>
            <svg-icon class="add-or-delete" icon-class="add-circle" @click="addRow($index)"  v-if="option.type!='singleInput'"/>
            <svg-icon class="add-or-delete" icon-class="minus-circle" @click="deleteRow($index)"  v-if="option.type!='singleInput'"/>
          </div>
        </template>
      </el-table-column>

      <el-table-column align="center" label="排序" width="80" v-if="option.type!='singleInput'">
        <template slot-scope="{row,$index}">
